#!/bin/bash
# AnEnemy smoke runner: build instrumented, run headless in the Playdate
...
BUNDLE="com.sdwfrost.anenemy"
DATA="$HOME/Developer/PlaydateSDK/Disk/Data/$BUNDLE"
# SDK 3.0.6: launch the .app via `open --args`, NOT the inner binary (the
# direct-binary form silently runs nothing on 3.0.6).
APP="$HOME/Developer/PlaydateSDK/bin/Playdate Simulator.app"
SHOT="$ROOT/build/anenemy-shot.png"
...
rm -rf "$DATA" "$SHOT"
open "$APP" --args "$ROOT/out/AnEnemySmoke.pdx" >/dev/null 2>&1
